...
# Replace CLOUD_HTML block
new_content = re.sub(r'(?s)CLOUD_HTML = r\"\"\"(.*?)</html>\n\"\"\"', new_cloud_html, content)

print("HTML replacement complete in memory!")

# Now let's update WebDashboardHandler to handle /github/login and /github/callback
handler_patch = """
        elif parsed_path.path == '/github/login':
            client_id = os.environ.get("GITHUB_CLIENT_ID", "")
            redirect_url = f"https://github.com/login/oauth/authorize?client_id={client_id}&scope=repo%20delete_repo"
...
new_content = new_content.replace("        elif parsed_path.path == '/video_feed':", handler_patch)

with open('ap copy.py', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Handler patch complete!")
